...
const maxDockerDaemonShutdownTimeoutSeconds = 120
...
func (a *Agent) runtimeStatus(ctx context.Context) RuntimeStatus {
	...
	if _, err := exec.LookPath("docker"); err != nil {
		...
		if managed, err := dockerDaemonForwardDropManaged(daemonConfigFile); err != nil {
			status.Errors["dockerDaemonForwardDrop"] = err.Error()
		} else if managed {
			status.DockerDaemonForwardDrop = true
		} else {
			status.Errors["dockerDaemonForwardDrop"] = "docker daemon must not disable FORWARD-chain default-drop management"
		}
		...
		if iccDisabled, err := dockerDaemonInterContainerCommunicationDisabled(daemonConfigFile); err != nil {
			status.Errors["dockerDaemonIcc"] = err.Error()
		} else if iccDisabled {
			status.DockerDaemonICCDisabled = true
		} else {
		...
		if bounded, err := dockerDaemonShutdownTimeoutBounded(daemonConfigFile); err != nil {
			status.Errors["dockerDaemonShutdownTimeout"] = err.Error()
		} else if bounded {
			status.DockerDaemonShutdownTimeoutBounded = true
		} else {
...
func dockerDaemonForwardDropManaged(path string) (bool, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return true, nil
	...
	raw, ok := config["ip-forward-no-drop"]
	if !ok {
		return true, nil
	}
	var disabled bool
	if err := json.Unmarshal(raw, &disabled); err != nil {
		return false, fmt.Errorf("Docker daemon config %q must be boolean", "ip-forward-no-drop")
	}
	return !disabled, nil
}
...
func dockerDaemonInterContainerCommunicationDisabled(path string) (bool, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return false, nil
...
func dockerDaemonShutdownTimeoutBounded(path string) (bool, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		if os.IsNotExist(err) {
			return true, nil
	...
	raw, ok := config["shutdown-timeout"]
	if !ok {
		return true, nil
	}
...
func verifyStartedContainerIsolation(ctx context.Context, plan DeploymentPlan) error {
	...
	if len(fields) > 48 && fields[48] != "none" {
		return fmt.Errorf("docker container %q has unexpected container ID file", plan.ContainerName)
	}
	if len(fields) > 49 && fields[49] != "0" {
		return fmt.Errorf("docker container %q has unexpected restart retry count", plan.ContainerName)
	}
