...
func verifyStartedContainerIsolation(ctx context.Context, plan DeploymentPlan) error {
	...
	if fields[11] != networkName {
		return fmt.Errorf("docker container %q attached to unexpected network %q", plan.ContainerName, fields[11])
	}
	networkCount, err := strconv.Atoi(fields[15])
	if err != nil || networkCount != 1 {
		return fmt.Errorf("docker container %q has unexpected network attachment count", plan.ContainerName)
	}
	attachedNetworks := strings.Split(fields[16], ",")
	...
	if !containerPortBindingsMatch(plan.Ports, fields[17]) {
	...
	securityOpts := strings.Split(fields[14], ",")
