...
func TestValidateDeploymentJobEnforcesAgentBoundary(t *testing.T) {
	cases := []struct {
		...
		{
			name: "empty image path component",
			edit: func(job *DeployJob) { job.Image = "registry.example.test//nginx:latest" },
			want: "invalid image reference",
		},
		{
			name: "empty image tag",
			edit: func(job *DeployJob) { job.Image = "nginx:" },
			want: "invalid image reference",
		},
		{
			name: "relative image path component",
			edit: func(job *DeployJob) { job.Image = "library/../nginx:latest" },
			want: "invalid image reference",
		},
		{
			name: "multiple image tag separators",
			edit: func(job *DeployJob) { job.Image = "nginx:latest:debug" },
			want: "invalid image reference",
		},
		{
			name: "image path component with tag separator",
			edit: func(job *DeployJob) { job.Image = "registry.example.test/team:dev/nginx:latest" },
			want: "invalid image reference",
		},
		{
...
func TestDockerSocketProtectedAllowsProtectedSymlinkParentDirectory(t *testing.T) {
	...
	if err != nil || !protected {
		t.Fatalf("expected protected symlinked Docker socket parent to pass, protected=%v err=%v", protected, err)
	}
}
...
func TestDockerRootDirProtectedAllowsProtectedSymlinkParentDirectory(t *testing.T) {
	tempDir := t.TempDir()
	realParent := filepath.Join(tempDir, "real-parent")
	if err := os.Mkdir(realParent, 0o700); err != nil {
	...
	if err != nil || !protected {
		t.Fatalf("expected protected symlinked Docker root parent to pass, protected=%v err=%v", protected, err)
	}
}
...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *json\ .Mounts*)
      ...
      ;;
    *.HostConfig.NanoCpus*)
      echo "1500000000 536870912 536870912 0 5g 67108864 json-file 10m 3 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1"
      exit 0
      ;;
...
func TestVerifyStartedContainerHealthcheckRequiresExpectedConfig(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "disabled",
			output:   `{"Test":["CMD-SHELL","curl -fsS http://127.0.0.1"],"Interval":30000000000,"Timeout":5000000000,"Retries":3,"Disable":true}`,
			contains: "disabled signed healthcheck",
		},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
...
func TestVerifyStartedContainerWorkloadRequiresSignedCustomLabels(t *testing.T) {
	cases := []struct {
		name     string
		output   string
		contains string
		...
		{
			name:     "missing",
			output:   `{"Config":{"Entrypoint":[],"Cmd":["sh","-lc","nginx -g 'daemon off;'"],"WorkingDir":"/","Labels":{"luma.managed":"true","luma.deployment":"dep_test","luma.tenant":"tenant_demo","luma.node":"node_local","luma.template":"tmpl_demo"},"Env":["LUMA_DEPLOYMENT_ID=dep_test","LUMA_NODE_ID=node_local","LUMA_TENANT_ID=tenant_demo"]}}`,
			contains: `expected Docker label "com.example.tier"`,
		},
		{
			name:     "drifted",
			output:   `{"Config":{"Entrypoint":[],"Cmd":["sh","-lc","nginx -g 'daemon off;'"],"WorkingDir":"/","Labels":{"luma.managed":"true","luma.deployment":"dep_test","luma.tenant":"tenant_demo","luma.node":"node_local","luma.template":"tmpl_demo","com.example.tier":"dev"},"Env":["LUMA_DEPLOYMENT_ID=dep_test","LUMA_NODE_ID=node_local","LUMA_TENANT_ID=tenant_demo"]}}`,
			contains: `drifted Docker label "com.example.tier"`,
		},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
...
`)
			...
			t.Setenv("DOCKER_WORKLOAD_OUTPUT", tt.output)
			job := sampleJob()
			job.Labels["com.example.tier"] = "prod"
			plan, err := deploymentPlan(job)
			if err != nil {
				t.Fatalf("deploymentPlan returned error: %v", err)
			}
...
func TestVerifyStartedContainerIsolationRejectsDuplicateSecurityOptions(t *testing.T) {
	tempDir := t.TempDir()
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none none 0 none none 0 0 SIGTERM /proc/acpi,/proc/kcore,/proc/keys,/proc/latency_stats,/proc/timer_list,/proc/sched_debug,/sys/firmware, /proc/asound,/proc/bus,/proc/fs,/proc/irq,/proc/sys,/proc/sysrq-trigger, 0"
  exit 0
fi
exit 1
`)
	...
	err = verifyStartedContainerIsolation(context.Background(), plan)
	if err == nil || !strings.Contains(err.Error(), "exact security options") {
		t.Fatalf("expected duplicate security option verification failure, got %v", err)
	}
}
...
func TestVerifyStartedContainerIsolationRejectsRestartRetryCount(t *testing.T) {
	tempDir := t.TempDir()
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none none none none none none none none none none 0 0 none none none 0 none none 0 0 SIGTERM /proc/acpi,/proc/kcore,/proc/keys,/proc/latency_stats,/proc/timer_list,/proc/sched_debug,/sys/firmware, /proc/asound,/proc/bus,/proc/fs,/proc/irq,/proc/sys,/proc/sysrq-trigger, 0 false none none none 3"
  exit 0
fi
exit 1
`)
	...
	err = verifyStartedContainerIsolation(context.Background(), plan)
	if err == nil || !strings.Contains(err.Error(), "restart retry count") {
		t.Fatalf("expected restart retry count verification failure, got %v", err)
	}
}
...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *json\ .Mounts*)
      ...
      ;;
    *.HostConfig.NanoCpus*)
      echo "1000000000 536870912 536870912 0 5g 67108864 json-file 10m 3 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1"
      exit 0
      ;;
    *.HostConfig.Privileged*)
      echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none none 0 none none 0 0 SIGTERM /proc/acpi,/proc/kcore,/proc/keys,/proc/latency_stats,/proc/timer_list,/proc/sched_debug,/sys/firmware, /proc/asound,/proc/bus,/proc/fs,/proc/irq,/proc/sys,/proc/sysrq-trigger, 0"
...
func TestVerifyStartedContainerResourcesRequiresSharedMemorySize(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "1500000000 536870912 536870912 0 5g 33554432 json-file 10m 3 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerResourcesRequiresLogBackpressurePolicy(t *testing.T) {
	cases := []struct {
		...
		{
			name:   "driver",
			output: "1500000000 536870912 536870912 0 5g 67108864 local 10m 3 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1",
		},
		{
			name:   "max-size",
			output: "1500000000 536870912 536870912 0 5g 67108864 json-file 100m 3 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1",
		},
		{
			name:   "max-file",
			output: "1500000000 536870912 536870912 0 5g 67108864 json-file 10m 10 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1",
		},
		{
			name:   "mode",
			output: "1500000000 536870912 536870912 0 5g 67108864 json-file 10m 3 blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1",
		},
		{
			name:   "max-buffer-size",
			output: "1500000000 536870912 536870912 0 5g 67108864 json-file 10m 3 non-blocking 32m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1",
		},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
...
func TestVerifyStartedContainerResourcesRejectsMissingStorageOptionCount(t *testing.T) {
	tempDir := t.TempDir()
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
...
`)
	...
	if err == nil || !strings.Contains(err.Error(), "unexpected field count") {
		t.Fatalf("expected missing storage option count verification failure, got %v", err)
	}
}

func TestVerifyStartedContainerResourcesRejectsExtraInspectFields(t *testing.T) {
	tempDir := t.TempDir()
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "1500000000 536870912 536870912 0 5g 67108864 json-file 10m 3 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1 unexpected"
  exit 0
fi
exit 1
`)
	...
	err = verifyStartedContainerResources(context.Background(), plan)
	if err == nil || !strings.Contains(err.Error(), "unexpected field count") {
		t.Fatalf("expected extra resource inspect field verification failure, got %v", err)
	}
}

func TestVerifyStartedContainerResourcesRejectsExtraLogOptionsWithStorageCount(t *testing.T) {
	tempDir := t.TempDir()
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "1500000000 536870912 536870912 0 5g 67108864 json-file 10m 3 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 5 1"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerResourcesRejectsSchedulerOverrides(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			output:   "1500000000 536870912 536870912 60 5g 67108864 json-file 10m 3 non-blocking 4m 0 0 0 0 none none 0 0 0 0 0 0 0 0 4 1",
