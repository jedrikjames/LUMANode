...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *.HostConfig.Privileged*)
      echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
      exit 0
      ;;
    *.State.Running*)
...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *.HostConfig.NanoCpus*)
      ...
      ;;
    *.HostConfig.Privileged*)
      echo "true true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
      exit 0
      ;;
    *.State.Running*)
      echo "true healthy true dep_test tenant_demo node_local"
...
func TestVerifyStartedContainerIsolationRequiresInit(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no false 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresPrivateUserNamespace(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private host private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresPrivatePidAndUTSNamespaces(t *testing.T) {
	cases := []struct {
		...
		{
			name:   "host-pid",
			output: "false true 512 none private private host private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none",
		},
		{
			name:   "host-uts",
			output: "false true 512 none private private private host no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none",
		},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
...
func TestVerifyStartedContainerIsolationRequiresExactCapabilityDropPolicy(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL,NET_RAW, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresExactSecurityOptions(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant,label=disable, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRejectsHostIntegrations(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "cap-add",
			output:   "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none NET_ADMIN, none 0 0 none none",
			contains: "added capabilities",
		},
		{
			name:     "group-add",
			output:   "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none docker, 0 0 none none",
			contains: "supplemental groups",
		},
		{
			name:     "devices",
			output:   "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 1 0 none none",
			contains: "host device access",
		},
		{
			name:     "device-requests",
			output:   "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 1 none none",
			contains: "host device access",
		},
		{
			name:     "volumes-from",
			output:   "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 dep_db, none",
			contains: "inherited host mounts",
		},
		{
			name:     "binds",
			output:   "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none /host:/container,",
			contains: "inherited host mounts",
		},
	}
	for _, tt := range cases {
...
func TestVerifyStartedContainerIsolationRequiresStopTimeout(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 5 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresAutoRemoveDisabled(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 30 true false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresOomKillEnabled(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 30 false false true luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none none none none none none none none none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresPublishAllPortsDisabled(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 30 false true false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRejectsLinksAndExtraHosts(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "links",
			output:   "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, dep_db:/db, none none none none none none none none none 0 0 none none",
			contains: "Docker links",
		},
		{
			name:     "extra-hosts",
			output:   "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none db.internal:10.0.0.5, none none none none none none none none 0 0 none none",
...
func TestVerifyStartedContainerIsolationRejectsDNSOverrides(t *testing.T) {
	cases := []struct {
		...
		{
			name:   "dns-server",
			output: "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none none 1.1.1.1, none none none none none none none 0 0 none none",
		},
		{
			name:   "dns-search",
			output: "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none none none example.internal, none none none none none none 0 0 none none",
		},
		{
			name:   "dns-options",
			output: "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none none none none ndots:0, none none none none none 0 0 none none",
		},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
...
func TestVerifyStartedContainerIsolationRejectsHostnameOverrides(t *testing.T) {
	cases := []struct {
		...
		{
			name:   "hostname",
			output: "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none none none none none custom-host none none none none 0 0 none none",
		},
		{
			name:   "domainname",
			output: "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none none none none none none example.internal none none none 0 0 none none",
		},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
...
func TestVerifyStartedContainerIsolationRejectsMacAddressOverride(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none none none none none none none 02:42:ac:11:00:02 none none 0 0 none none"
  exit 0
fi
exit 1
`)
...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *.HostConfig.NanoCpus*)
      ...
      ;;
    *.HostConfig.Privileged*)
      echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 2 luma-tenant_demo,bridge, none none none none none none none none none none none 0 0 none none"
      exit 0
      ;;
    *.State.Running*)
      echo "true healthy true dep_test tenant_demo node_local"
...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *.HostConfig.NanoCpus*)
      ...
      ;;
    *.HostConfig.Privileged*)
      echo "false true 512 none private private private private no true 30 false false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none none none none none none none none none 0 0 none none"
      exit 0
      ;;
    *.State.Running*)
      echo "true healthy true dep_test tenant_demo node_local"
