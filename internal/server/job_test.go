...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *.HostConfig.Privileged*)
      echo "false true 512 none private no true 30 false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none"
      exit 0
      ;;
    *.State.Running*)
...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *.HostConfig.NanoCpus*)
      ...
      ;;
    *.HostConfig.Privileged*)
      echo "true true 512 none private no true 30 false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none"
      exit 0
      ;;
    *.State.Running*)
      echo "true healthy true dep_test tenant_demo node_local"
...
func TestVerifyStartedContainerIsolationRequiresInit(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private no false 30 false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresStopTimeout(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private no true 5 false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresAutoRemoveDisabled(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private no true 30 true false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRequiresPublishAllPortsDisabled(t *testing.T) {
	...
	writeFakeCommand(t, tempDir, "docker", `#!/bin/sh
if [ "$1" = "inspect" ]; then
  echo "false true 512 none private no true 30 false true luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none"
  exit 0
fi
exit 1
`)
...
func TestVerifyStartedContainerIsolationRejectsLinksAndExtraHosts(t *testing.T) {
	cases := []struct {
		...
		{
			name:     "links",
			output:   "false true 512 none private no true 30 false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, dep_db:/db, none",
			contains: "Docker links",
		},
		{
			name:     "extra-hosts",
			output:   "false true 512 none private no true 30 false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, 80/tcp=8080;, none db.internal:10.0.0.5,",
...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *.HostConfig.NanoCpus*)
      ...
      ;;
    *.HostConfig.Privileged*)
      echo "false true 512 none private no true 30 false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=lumapanel-default,apparmor=lumapanel-tenant, 2 luma-tenant_demo,bridge, none none none"
      exit 0
      ;;
    *.State.Running*)
      echo "true healthy true dep_test tenant_demo node_local"
...
if [ "$1" = "inspect" ]; then
  case "$3" in
    ...
    *.HostConfig.NanoCpus*)
      ...
      ;;
    *.HostConfig.Privileged*)
      echo "false true 512 none private no true 30 false false luma-tenant_demo 10000:10000 ALL, no-new-privileges=true,seccomp=default,apparmor=lumapanel-tenant, 1 luma-tenant_demo, none none none"
      exit 0
      ;;
    *.State.Running*)
      echo "true healthy true dep_test tenant_demo node_local"
